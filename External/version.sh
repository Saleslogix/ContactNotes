...
build_git_version() {
	...
	git_version_short=`git describe --tags --abbrev=0`
	if [ "$git_version" != "$git_version_short" ]; then
		git_version=${git_version#`echo $git_version_short`-}
		git_version=`echo $git_version | sed s/-/./g`
		git_version=$git_version_short+$git_version
	fi
	echo $git_version
}
